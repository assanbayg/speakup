...
class SpriteService {
  ...
  static Future<List<String>> listApprovedSprites(String userId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return List<String>.from(data['sprites'] ?? []);
      } else {
  ...
  /// List pending (under review) sprites for a user
  static Future<List<String>> listPendingSprites(String userId) async {
    try {
      final uri = Uri.parse('$_baseUrl/sprites/my-pending')
          .replace(queryParameters: {'user_id': userId});
      final response = await http.get(uri).timeout(const Duration(seconds: 10));
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        return List<String>.from(data['pending'] ?? []);
      } else {
        return [];
      }
    } catch (e) {
  ...
  /// Get the URL for an approved sprite image
  ...
  static Future<Uint8List?> getSpriteBytes(
    ...
    try {
      ...
      if (response.statusCode == 302 || response.statusCode == 307) {
        ...
        if (redirectUrl != null) {
          final r = await http.get(Uri.parse(redirectUrl));
